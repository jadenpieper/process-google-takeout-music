...
# source = Song Title
# TODO: Replace globals with config.yaml
PARENT_TAKEOUT = (
    "/Users/jadenpieper/Documents/Projects/google-takeout-music/GoogleTakeout"
)
...
def song2album(songpath, df):
    ...
    if album is None or artist is None:
        ...
        updated_song = re.sub(duplicate_pattern, "", songname + ".mp3").replace(
            ".mp3", ""
        )
        ...
        if "_" in songname:
            ...
            for mark in replace_marks:
                mark_match = df["Song Title"].str.contains(
                    songname.replace("_", mark), regex=False
                )
        ...
        dup_ix = match_df.duplicated(
            subset=["Song Title", "Album Title", "Artist Names"]
        )
...
def move_song(songpath, df):
    ...
    if os.path.exists(dst_path):
        ...
        if dst_bitrate > src_bitrate:
            print(
                f"Not overwriting {dst_path} with {songpath}, has higher bitrate ({dst_bitrate} > {src_bitrate}"
            )
            do_copy = False
    if do_copy:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--idx", type=int, default=0, help="Starting index for album selection"
    )
    ...
    if args.album_select:
        album_selection(
            ALBUMPATH, output_path=SAVEPATH, idx=args.idx, start_album=args.start_album
        )
